...
void CAN_FILTER_INIT(void)
{
    ...
    sFilterConfig.FilterMode = CAN_FILTERMODE_IDMASK; // 过滤器模式为ID屏蔽位模式
...
uint8_t CAN_SEND_MSG(uint8_t *msg, uint8_t len)
{
    ...
    if (HAL_CAN_AddTxMessage(&hcan1, &TX_HEADER, message, &TxMailbox) != HAL_OK) // 发送
    {
        return 1;
    }
...
//         RX_BUFFER[i] = RX_DATA[i]; // 用RxBuf转存RxData的数据
//     }
// }
/* USER CODE END 4 */
